...
func spawn() -> void :
	...
	if spawned_object.has_signal("spawned_child"):
		spawned_object.connect("spawned_child", self, "object_spawned_child")
	spawned_object.connect("zero_health", self, "on_object_death")
	spawned_object.connect("death", self, "emit_signal", ["actual_enemy_death"])
