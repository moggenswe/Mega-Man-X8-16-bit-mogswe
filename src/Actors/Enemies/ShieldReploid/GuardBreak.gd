...
func shield_throw() -> void :
	...
	tween.tween_property(flying_shield, "position", Vector2(0, - jump_velocity), travel_time / 2).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CUBIC)
	tween.tween_property(flying_shield, "position", Vector2(0, 0), travel_time / 2).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_SINE)
	tween.tween_callback(self, "catch_shield")
