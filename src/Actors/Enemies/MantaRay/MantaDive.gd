...
func _Update(_delta) -> void :
	if attack_stage == 0:
		start_tween()
		tween.tween_property(character, "position:y", - 10.0, 0.5).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_CUBIC)
		tween.tween_property(character, "position", get_target(), 1.35).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_CUBIC)
