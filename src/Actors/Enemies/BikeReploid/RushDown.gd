...
func _Setup() -> void :
	tween = create_tween()
	tween.tween_method(self, "force_movement", get_actual_speed(), horizontal_velocity / 1.25, duration / 2)
...
func _Update(_delta) -> void :
	...
	if attack_stage == 0 and timer > 0.5:
		tween = create_tween()
		tween.tween_method(self, "force_movement", get_actual_speed(), horizontal_velocity, duration)
