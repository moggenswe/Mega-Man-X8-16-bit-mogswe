...
static func filter_for_type(type, children):
	var particle_list = []
...
static func filter_particles(children):
	var particle_list = []
	for child in children:
...
static func filter_for_name(text: String, children):
	var valid_children = []
	for child in children:
...
static func tween(parent, property, final_value, duration, ease_type: = Tween.EASE_IN_OUT, trans_type: = Tween.TRANS_LINEAR) -> void :
	...
	final_value = TweenController.match_property_type(parent, property, final_value)
