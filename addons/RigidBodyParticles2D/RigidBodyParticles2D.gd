...
export (Texture) var alternate_texture
...
func _ready():
	randomize()
	if get_node("Explosion Particles"):
...
func _physics_process(delta):
	...
	for i in frame_emit_count:
		...
		if _iteration == 0 and particle.get_class() != "RigidBody2D":
		...
		if alternate_texture:
			particle.get_node("sprite").texture = alternate_texture
		add_child(particle)
		_emit_count += 1
